...
void Renderer::nanogui_init(GLFWwindow* window)
{
	...
	gui_1->addGroup("Bone Rotation");
	 ...
	 rotate_x1 = gui_1->addVariable("Rotate X1", m_bone_animation->yellowXangle);

		std::cout << typeid(rotate_x1).name() << std::endl;

	//static auto rotate_x2 = gui_1->addVariable("Rotate X2", m_bone_animation->pink[0]);
	//static auto rotate_x3 = gui_1->addVariable("Rotate X3", m_bone_animation->blue[0]);
	 rotate_y1 = gui_1->addVariable("Rotate Y1", m_bone_animation->yellowYangle);
	//static auto rotate_y2 = gui_1->addVariable("Rotate Y2", m_bone_animation->pink[1]);
	//static auto rotate_y3 = gui_1->addVariable("Rotate Y3", m_bone_animation->blue[1]);
	 rotate_z1 = gui_1->addVariable("Rotate Z1", m_bone_animation->yellowZangle);


	 rotate_x2 = gui_1->addVariable("Rotate X2", m_bone_animation->purpleXangle);
	//static auto rotate_x2 = gui_1->addVariable("Rotate X2", m_bone_animation->pink[0]);
	//static auto rotate_x3 = gui_1->addVariable("Rotate X3", m_bone_animation->blue[0]);
	 rotate_y2 = gui_1->addVariable("Rotate Y2", m_bone_animation->purpleYangle);
	//static auto rotate_y2 = gui_1->addVariable("Rotate Y2", m_bone_animation->pink[1]);
	//static auto rotate_y3 = gui_1->addVariable("Rotate Y3", m_bone_animation->blue[1]);
	 rotate_z2 = gui_1->addVariable("Rotate Z2", m_bone_animation->purpleZangle);
	/*static auto rotate_z2 = gui_1->addVariable("Rotate Z2", m_bone_animation->pink[2]);
	static auto rotate_z3 = gui_1->addVariable("Rotate Z3", m_bone_animation->blue[2]);*/
	 rotate_x3 = gui_1->addVariable("Rotate X3", m_bone_animation->blueXangle);
	//static auto rotate_x2 = gui_1->addVariable("Rotate X2", m_bone_animation->pink[0]);
	//static auto rotate_x3 = gui_1->addVariable("Rotate X3", m_bone_animation->blue[0]);
	 rotate_y3 = gui_1->addVariable("Rotate Y3", m_bone_animation->blueYangle);
	//static auto rotate_y2 = gui_1->addVariable("Rotate Y2", m_bone_animation->pink[1]);
	//static auto rotate_y3 = gui_1->addVariable("Rotate Y3", m_bone_animation->blue[1]);
	 rotate_z3 = gui_1->addVariable("Rotate Z3", m_bone_animation->blueZangle
		);
	
	//gui_1->addButton("Reset Bone Rotation", []() {
	//	m_bone_animation->reset();
	//	rotate_x1->setValue(m_bone_animation->yellow[0]);
	//	rotate_x2->setValue(m_bone_animation->pink[0]);
	//	rotate_x3->setValue(m_bone_animation->blue[0]);
	//	rotate_y1->setValue(m_bone_animation->yellow[1]);
	//	rotate_y2->setValue(m_bone_animation->pink[1]);
	//	rotate_y3->setValue(m_bone_animation->blue[1]);
	//	rotate_z1->setValue(m_bone_animation->yellow[2]);
	//	rotate_z2->setValue(m_bone_animation->pink[2]);
	//	rotate_z3->setValue(m_bone_animation->blue[2]);
	//});

	gui_1->addButton("Reset", []() {
		m_bone_animation->yellowXangle = 0;
		m_bone_animation->yellowYangle = 0;
		m_bone_animation->yellowZangle = 0;

		m_bone_animation->purpleXangle = 0;
		m_bone_animation->purpleYangle = 0;
		m_bone_animation->purpleZangle = 0;

		m_bone_animation->blueXangle = 0;
		m_bone_animation->blueYangle = 0;
		m_bone_animation->blueZangle = 0;


		//rotate_y1->setValue(m_bone_animation->yellowYangle);
		//rotate_z1->setValue(m_bone_animation->yellowZangle);

		//rotate_x2->setValue(m_bone_animation->purpleXangle);
		//rotate_y2->setValue(m_bone_animation->purpleYangle);
		//rotate_z2->setValue(m_bone_animation->purpleZangle);

		//rotate_x3->setValue(m_bone_animation->blueXangle);
		//rotate_y3->setValue(m_bone_animation->blueYangle);
		//rotate_z3->setValue(m_bone_animation->blueZangle);
	});
 gui_1->addVariable("Bone Move: ", m_bone_animation->is_bone_move);
	...
	currentX =	gui_1->addVariable("Current X", m_bone_animation->blue_position_end.x);
	currentY = gui_1->addVariable("Current Y", m_bone_animation->blue_position_end.y);
	currentZ = gui_1->addVariable("Current Z", m_bone_animation->blue_position_end.z);


	

	//gui_1->addVariable("Current end pos X", m_bone_animation->end_position[0]);
	//gui_1->addVariable("Current end pos Y", m_bone_animation->end_position[1]);
	//gui_1->addVariable("Current end pos Z", m_bone_animation->end_position[2]);
...
void Renderer::display(GLFWwindow* window)
{
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		rotate_x1->setValue(m_bone_animation->angles[6]);
		rotate_x2->setValue(m_bone_animation->angles[7]);
		rotate_x3->setValue(m_bone_animation->angles[8]);

		rotate_y1->setValue(m_bone_animation->angles[3]);
		rotate_y2->setValue(m_bone_animation->angles[4]);
		rotate_y3->setValue(m_bone_animation->angles[5]);


		rotate_z1->setValue(m_bone_animation->angles[0]);
		rotate_z2->setValue(m_bone_animation->angles[1]);
		rotate_z3->setValue(m_bone_animation->angles[2]);


		currentX->setValue(m_bone_animation->blue_position_end.x);
		currentY->setValue(m_bone_animation->blue_position_end.y);
		currentZ->setValue(m_bone_animation->blue_position_end.z);


	/*	if (!m_bone_animation->is_bone_move) {
			m_bone_animation->forward_kinematics(m_bone_animation->yellowXangle, m_bone_animation->yellowYangle, m_bone_animation->yellowZangle, m_bone_animation->purpleXangle, m_bone_animation->purpleYangle, m_bone_animation->purpleZangle
				, m_bone_animation->blueXangle, m_bone_animation->blueYangle, m_bone_animation->blueZangle);
		}*/
...
void Renderer::draw_bones(Shader& shader, Bone_Animation* m_bone_animation)
{
	...
	// Draw blue bone
	glm::mat4 blue_bone_obj_mat = m_bone_animation->get_blue_mat();
